...
/**
 * Cloud Function to analyze resume against job description with streaming
 */
export const analyzeStream = onRequest(
  {
    timeoutSeconds: 540, // 9 minutes max for streaming
    memory: '1GiB',
    cors: true
  },
  async (req, res) => {
    ...
    try {
      ...
      if (!resumeText || !jobDescription) {
        res.status(400).json({ error: 'Missing required fields: resumeText and jobDescription' });
        return;
      }

      // @ts-ignore - Complex type inference causes TS error
